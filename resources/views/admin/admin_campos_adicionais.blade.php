		...
		@if(Request::is('painel_de_controle/admin/*/campos_adicionais_editar'))
	            	...
	            	<table class="table table-striped table-sm">
		                ...
		                <tbody>
		                  @foreach ($campo as $row)
		                   <tr>
		                   	{!! Form::open(['method' =>'POST', 'url' =>'painel_de_controle/admin/'.$row->extra_id.'/campos_adicionais_atualizar'])!!}
		                    <td>{{ $row->nome }}</td>
		...
		@elseif(Request::is('painel_de_controle/admin/*/campos_adicionais_confirmar_deletar'))
          		...
          		<div class="table-responsive">
	            	<table class="table table-striped table-sm">
		                ...
		                <tbody>
		                  @foreach ($campo as $row)
		                   <tr>
		                   	{!! Form::open(['method' =>'DELETE', 'url' =>'painel_de_controle/admin/'.$row->extra_id.'/campos_adicionais_deletar'])!!}
		                    <td>{{ $row->nome }}</td>
		                    <td>{{ $row->departamento }}</td>
		                    <td>{{ $row->empresa }}</td>
		                    <td>{{ $row->campo }}
	            	...
	            	<table class="table table-striped table-sm">
		                ...
		                <tbody>
		                  @foreach ($eventos as $row)
		                   <tr>
		                   	{!! Form::open(['method' =>'POST', 'url' =>'painel_de_controle/admin/campos_adicionais_cadastrar'])!!}
		                    <td>{{ $row->nome }}</td>
		                    <td>{{ $row->departamento }}</td>
		                    <td>{{ $row->empresa }}</td>
		                    <td>
		...
		@endif     
          ...
          <div class="margem_cima_baixo">
          	<h4>Campos adicionais dos eventos</h4>
		                ...
		                <tbody>
		                  @foreach ($campos_adicionais as $campo)
		                   <tr>
		                    ...
		                    <td>{{ $campo->campo }}</td>
                            <td>
                              {!! Form::open(['method' =>'POST', 'url' =>'painel_de_controle/admin/'.$campo->extra_id.'/campos_adicionais_editar'])!!}
                              {!! Form::submit('Editar', ['class'=>'btn btn-primary']) !!}
                              {!! Form::close() !!}
                            </td>
                            <td>
                              {!! Form::open(['method' =>'POST', 'url' =>'painel_de_controle/admin/'.$campo->extra_id.'/campos_adicionais_confirmar_deletar'])!!}
                              {!! Form::submit('DELETAR', ['class'=>'btn btn-danger ']) !!}
                              {!! Form::close() !!}
                            </td>
